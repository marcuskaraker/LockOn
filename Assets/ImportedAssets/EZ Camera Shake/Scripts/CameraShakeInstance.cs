        ...
        public Vector3 UpdateShake()
        {
            ...
            if (fadeInDuration > 0 && sustain)
            {
                if (currentFadeTime < 1)
                    currentFadeTime += Time.unscaledDeltaTime / fadeInDuration;
                else if (fadeOutDuration > 0)
                    sustain = false;
            }

            if (!sustain)
                currentFadeTime -= Time.unscaledDeltaTime / fadeOutDuration;

            if (sustain)
                tick += Time.unscaledDeltaTime * Roughness * roughMod;
            else
                tick += Time.unscaledDeltaTime * Roughness * roughMod * currentFadeTime;
